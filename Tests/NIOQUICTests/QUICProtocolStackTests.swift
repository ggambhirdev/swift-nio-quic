...
final class QUICProtocolStackTests: XCTestCase {
    ...
    func testCurrentMetricsBeforeEstablishment() throws {
        let eventLoop = EmbeddedEventLoop()
        let udpChannel = EmbeddedChannel(loop: eventLoop)
        defer { XCTAssertNoThrow(try udpChannel.finish()) }
        ...
        let connection = try SwiftNetworkQUICConnection<QUICStreamChannels>.server(
            ...
            remoteAddress: try SocketAddress(ipAddress: "127.0.0.1", port: 5678),
    ...
    func testCurrentMetricsAfterTrafficAndClose() async throws {
        ...
        // Bound handshake, stream exchange and metrics retrieval even if a regression
        // prevents the peer from responding. Closing the transports unblocks stream waits.
        let serverDeadline = serverChannel.eventLoop.scheduleTask(in: .seconds(15)) {
            XCTFail("Live metrics test exceeded its deadline")
            serverChannel.close(promise: nil)
            clientChannel.close(promise: nil)
        }
        defer { serverDeadline.cancel() }

        let clientConnection = try await clientMultiplexer.createNewConnection(
            serverName: serverChannel.localAddress!.ipAddress!,
            remoteAddress: serverChannel.localAddress!,
            inboundStreamInitializer: { channel in
                channel.eventLoop.makeCompletedFuture { fatalError("Unexpected inbound stream") }
            }
        )
        let connectionBox = NIOLockedValueBox<QUICConnectionChannel<QUICStreamChannels>?>(nil)
        ...
        try await withThrowingTaskGroup(of: Void.self) { group in
            ...
            do {
                ...
                for exchange in 0..<2 {
                    let stream = try await clientConnection.createBidirectionalStream { initializer in
                        initializer.channel.eventLoop.makeCompletedFuture {
                            let connection = try XCTUnwrap(
                                initializer.channel.parent as? QUICConnectionChannel<QUICStreamChannels>
                            )
                    ...
                    let payload = ByteBuffer(string: "metrics exchange \(exchange)")
                    try await stream.executeThenClose { inbound, outbound in
                    ...
                    let connection = try XCTUnwrap(connectionBox.withLockedValue { $0 })
                    ...
                    XCTAssertGreaterThanOrEqual(metrics.minimumRTT, .zero)
                    XCTAssertGreaterThanOrEqual(metrics.smoothedRTT, .zero)
                    XCTAssertGreaterThanOrEqual(metrics.rttVariance, .zero)
                    // Exclude SwiftNetwork's unmeasured minimum sentinel. A response
                    // acknowledges earlier traffic, so this is a measured connection.
                    XCTAssertLessThan(metrics.minimumRTT, .seconds(Int64(UInt32.max)))

                    let onLoop = try await connection.eventLoop.flatSubmit {
                ...
                // A task group waits for its children before leaving this scope.
                // Close here as well as in the outer defers to release a waiting echo task.
                serverChannel.close(promise: nil)
                clientChannel.close(promise: nil)
                group.cancelAll()
                throw error
